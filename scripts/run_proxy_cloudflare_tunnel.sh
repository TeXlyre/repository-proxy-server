...
echo -e "${YELLOW}Starting Cloudflare Tunnel Setup for Repository Proxy...${NC}"
...
if [ -n "$EXISTING_TUNNEL" ]; then
    ...
    if [ ! -f "$CREDENTIALS_DIR/$TUNNEL_ID.json" ]; then
        ...
        if [ -z "$TUNNEL_ID" ]; then
            ...
            read -p "Please enter the tunnel ID manually: " TUNNEL_ID
            if [ -z "$TUNNEL_ID" ]; then
...
echo -e "${YELLOW}Repository proxy accessible at: https://$HOSTNAME${NC}"
